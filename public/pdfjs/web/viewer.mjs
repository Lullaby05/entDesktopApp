...
class PasswordPrompt {
  ...
  async open() {
    ...
    this.#activeCapability = withResolvers();
    try {
      await this.overlayManager.open(this.dialog);
    } catch (ex) {
...
class PDFAttachmentViewer extends BaseTreeViewer {
  ...
  reset(keepRenderedCapability = false) {
    ...
    if (!keepRenderedCapability) {
      this._renderedCapability = withResolvers();
    }
...
class PDFDocumentProperties {
  ...
  #reset() {
    this.pdfDocument = null;
    this.#fieldData = null;
    this._dataAvailableCapability = withResolvers();
...
class PDFFindController {
  ...
  #reset() {
    ...
    clearTimeout(this._findTimeout);
    this._findTimeout = null;
    this._firstPageCapability = withResolvers();
  }
  get #query() {
...
class PDFOutlineViewer extends BaseTreeViewer {
  ...
  _dispatchEvent(outlineCount) {
    this._currentOutlineItemCapability = withResolvers();
    if (
      outlineCount === 0 ||
  ...
  async _getPageNumberToDestHash(pdfDocument) {
    ...
    this._pageNumberToDestHashCapability = withResolvers();
...
class PDFScriptingManager {
  ...
  async dispatchWillPrint() {
    ...
    this.#willPrintCapability = withResolvers();
    try {
      await this.#scripting.dispatchEventInSandbox({
        id: 'doc',
        name: 'WillPrint',
  ...
  async #dispatchPageOpen(pageNumber, initialize = false) {
    ...
    if (initialize) {
      this.#closeCapability = withResolvers();
    }
  ...
  #initScripting() {
    this.#destroyCapability = withResolvers();
    if (this.#scripting) {
...
class PDFViewer {
  ...
  async #onePageRenderedOrForceFetch(signal) {
    ...
    const hiddenCapability = withResolvers();
  ...
  _resetView() {
    ...
    this._firstPageCapability = withResolvers();
    this._onePageRenderedCapability = withResolvers();
    this._pagesCapability = withResolvers();
