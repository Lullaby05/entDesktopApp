  ...
  <div class="container">
    <div class="guide">
      <a-space :size="[24, 0]">
        <div
          ...
          @click.stop="getEnterpriseCertificateData(1)"
        >
          <div class="number">{{ enterpriseCount.allNum || 0 }}</div>
          <div class="text">全部证照</div>
        </div>
        <div
          class="item success"
          :class="selectForm.queryCondition === 2 ? 'active' : ''"
          @click.stop="getEnterpriseCertificateData(2)"
        >
          <div class="number">{{ enterpriseCount.validNum || 0 }}</div>
          <div class="text">有效证照</div>
        </div>
        <div
          class="item danger"
          :class="selectForm.queryCondition === 3 ? 'active' : ''"
          @click.stop="getEnterpriseCertificateData(3)"
        >
          <div class="number">{{ enterpriseCount.invalidNum || 0 }}</div>
          <div class="text">无效证照</div>
        </div>
        <div
          class="item warning"
          :class="selectForm.queryCondition === 4 ? 'active' : ''"
          @click.stop="getEnterpriseCertificateData(4)"
        >
          <div class="number">{{ enterpriseCount.adventNum || 0 }}</div>
          <div class="text">临期证照</div>
        </div>
        <div
          class="item warning"
          :class="selectForm.queryCondition === 5 ? 'active' : ''"
          @click.stop="getEnterpriseCertificateData(5)"
        >
          <div class="number">{{ enterpriseCount.waitReviewNum || 0 }}</div>
    ...
    <TableListCard>
      ...
      <template #primaryBtn>
        <a-space>
          <a-button
            v-permission="'EnterpriseCollectAddPermission'"
            type="primary"
      ...
      <template #table>
        <a-table
          :data="enterpriseCertificateData.list"
          row-key="id"
          :columns="columns"
          :bordered="false"
          :pagination="false"
        >
          ...
          <template #option="{ record }">
            <a-button
              v-permission="'EnterpriseCollectEditPermission'"
              type="text"
              size="mini"
            ...
            <a-button
              v-permission="'EnterpriseCollectDeletePermission'"
              type="text"
              status="danger"
              size="mini"
      ...
      <template #pagination>
        <a-pagination
          ...
          :base-size="3"
          :buffer-size="1"
          :total="(enterpriseCertificateData.total as number)"
...
<script lang="ts">
  ...
  import {
    ...
    deleteEnterpriseCertificateAPI,
    getEnterpriseCountAPI,
  ...
  import type { GetEnterpriseCertificateList } from '@/api/enterpriseManage';
...
<script lang="ts" setup>
  ...
  // ---------------- 前置数据 --------------------
  const enterpriseCount = ref<any>({});

  const getEnterpriseCount = async () => {
    const { data } = await getEnterpriseCountAPI();
    enterpriseCount.value = data;
  };
  ...
  const getEnterpriseCertificateData = async (queryCondition?: number) => {
    if (queryCondition) {
    ...
    enterpriseCertificateData.value = res;
  };
  ...
  const enterpriseCertificateData = ref<EnterpriseCetificateData>({
    list: [],
    total: 0,
  });
  ...
  const deleteRow = (id: string, name: string) => {
    Modal.open({
      ...
      titleAlign: 'start',
      onOk: async () => {
        await deleteEnterpriseCertificateAPI(id);
        Message.success({
          content: '删除成功',
        });
        let totalPage = Math.ceil(
          (enterpriseCertificateData.value.total! - 1) /
            selectForm.value.pageSize!
        );
        let currentPage =
