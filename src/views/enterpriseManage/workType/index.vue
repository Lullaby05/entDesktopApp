    ...
    <TableListCard>
      <template #headerInput>
        <a-form ref="selectFormDom" :model="tempSearchForm" layout="inline">
          <a-form-item field="name" label="工种名称">
            <a-input
          ...
          <a-form-item field="odDiseasesTypeText" label="相关职业病">
            <a-input
              v-model:model-value="tempSearchForm.odDiseasesTypeText"
              placeholder="请输入相关职业病"
              allow-clear
            ></a-input>
          </a-form-item>
        </a-form>
...
<script lang="ts">
  ...
  import type { WorkTypeData } from './index';
...
<script lang="ts" setup>
  ...
  const getWorkTypeData = async () => {
    ...
    res.list &&
      res.list.length &&
      res.list.forEach((e) => {
        e.odDiseasesTypeText = e.odDiseasesTypeText.replace(/,/g, '，');
        e.odDiseasesTypeText = e.odDiseasesTypeText.replace(/，*$/, '');
      });
