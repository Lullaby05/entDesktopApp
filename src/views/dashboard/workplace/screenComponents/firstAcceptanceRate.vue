<!-- 一次验收合格率 -->
<template>
  <div class="first-acceptance-rate-container">
    <div class="first-acceptance-chart" ref="echartsRef"> </div>
    <div class="description-text">
      <span class="rate-text">一次验收合格率</span>
    </div>
  </div>
</template>
<script lang="ts" setup>
  ...
  const currentData = computed<any>(() => {
    return (
      ...
      Math.round(data.value['riskGovern'].onceQualifiedRate)
    );
  });
...
<style lang="less" scoped>
  .first-acceptance-rate-container {
    width: 11rem;
    height: 100%;
    display: flex;
    align-items: center;
