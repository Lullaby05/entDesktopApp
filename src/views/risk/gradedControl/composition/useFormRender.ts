...
export function useFormRender(record?: any): returnObj {
  ...
  if (!record) {
    ...
    formInfo = { ...storeDesign.state.design };
  } else {
    // 传了就需要看formId和传过来的是否相同，相同代表编辑的时候表单被获取了可能被修改了，修改后会存到pinia中，以pinia中的为准
    if (record.formId === (storeDesign.state.design as any).formId) {
      formInfo = { ...record, ...storeDesign.state.design };
  ...
  const editFormRender = () => {
    if (record && Object.keys(record).length) {
      storeDesign.commit('loadForm', record);
    }
    router.push({
