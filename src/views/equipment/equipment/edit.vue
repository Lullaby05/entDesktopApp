            ...
            <a-col :span="10">
              <a-form-item label="是否年检" field="annualinspection">
                <a-radio-group v-model="form.annualinspection" @change="radio">
                  <a-radio :value="true">需要年检</a-radio>
                  <a-radio :value="false">不需要年检</a-radio>
                </a-radio-group>
              </a-form-item>
            </a-col>
          </a-row>
          ...
          <a-row v-if="form.annualinspection" :gutter="16">
            <a-col :span="10">
              <a-form-item label="登记编号" field="registrationNumber">
                <a-input
                  v-model="equipmentCertificateUpdateReqVO.registrationNumber"
                  class="input-cell"
                  allow-clear
                  placeholder="登记编号"
                />
            ...
            <a-col :span="10">
              <a-form-item label="登记证书" field="registrationCertificate">
                <a-input
                  v-model="
                    equipmentCertificateUpdateReqVO.registrationCertificate
                  "
                  class="input-cell"
                  allow-clear
                  placeholder="登记证书"
            ...
            <a-col :span="20">
              ...
              <a-form-item label-col-flex="1px">
                <a-upload
                  ...
                  action="/dev/businessApi/business/upload/uploadFile"
                  :file-list="fileList"
                  image-preview
...
<script lang="ts" setup>
  ...
  import { update, typeList, getDetail } from '@/api/equipment';
  ...
  const equipmentCertificateUpdateReqVO = ref<any>({});
  ...
  const checkDateChange = (
    ...
    equipmentCertificateUpdateReqVO.value.checkDate = dateString;
  };
  const nextCheckDateChange = (
    ...
    equipmentCertificateUpdateReqVO.value.nextCheckDate = dateString;
  };
  ...
  const radio = (value: string | number | boolean) => {
    ...
    if (value === 0) {
      equipmentCertificateUpdateReqVO.value = {
        registrationNumber: '',
  ...
  const createClick = async () => {
    ...
    if (fileList.value.length > 0) {
      equipmentCertificateUpdateReqVO.value.accessory = fileList.value
        .map((item) => {
          if (item.response) {
            return item.response.data[0].fileName;
          }
  ...
  const startPictureList = (file: any[]) => {
    if (file.length === 0) {
      return;
    }
    file.forEach((item: any, index: number) => {
      const name = item
        .split('rw8my19lv.hn-bkt.clouddn.com/')[1]
        .split('?e=')[0];
      const fileItem: any = {
        uid: index, // 文件UID （唯一标识一个文件）
        url: item,
        name,
      };
  ...
  const startNamePlatList = (file: any[]) => {
    if (file.length === 0) {
      return;
    }
    file.forEach((item: any, index: number) => {
      ...
      namePlatList.value.push(fileItem);
    });
  };

  const startAccessoryList = (file: any[]) => {
    if (file.length === 0) {
      return;
    }
    file.forEach((item: any, index: number) => {
      ...
      accessoryList.value.push(fileItem);
    });
  };
  ...
  const detail = async (id: string) => {
    ...
    form.value = res.data;
    equipmentCertificateUpdateReqVO.value =
      form.value.equipmentCertificateUpdateReqVORespVO;
    startPictureList(form.value.pictureList);
    if (equipmentCertificateUpdateReqVO.value) {
      startFileList(equipmentCertificateUpdateReqVO.value.accessoryList);
    }
    startNamePlatList(form.value.namePlatList);
    startAccessoryList(form.value.accessoryList);
  };

  watch(
    () => router.currentRoute.value.path,
    (toPath) => {
      if (toPath === '/equipment/equipmentEdit') {
