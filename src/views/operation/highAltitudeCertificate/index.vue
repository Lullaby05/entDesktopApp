    ...
    <TableListCard>
      <template #headerInput>
        <a-form :model="tempSearchForm" layout="inline">
          ...
          <a-form-item field="operationLevel" label="高处作业级别">
            <a-select
      ...
      <template #primaryBtn>
        <a-button
          v-permission="'HighAltitudeCertificateAdd'"
          type="primary"
          @click="handleAdd"
        ...
        >
          ...
          <template #certificateStatus="{ record }">
            <div
              class="color-red"
              :class="
                record.status === blindPlateStatusEnum.已关闭
            ...
            >
              <span
                >{{ record.statusName
                  ...
                  record.status !== blindPlateStatusEnum.已撤回 &&
                  record.timeStatus
                    ? '（已超时）'
                    : ''
                }}</span
          ...
          <template #option="{ record }">
            <div>
              ...
              >
              <a-button
                v-permission="'HighAltitudeCertificateHandling'"
                type="text"
                size="mini"
...
<script lang="ts" setup name="HighAltitudeCertificateIndex">
  ...
  getWorkUnitList(operationTypeEnum.HIGHALTITUDE).then((res: any) => {
  ...
  getOperationTypeEnum(operationTypeEnum.HIGHALTITUDE).then((res) => {
    fireStatusDict.value = changeMapToDict(res.data);
  });
  getMapDictionaryAPI('H_LEVEL').then((res) => {
  ...
  const getTableList = async () => {
    const { data } = await getCertificateList(operationTypeEnum.HIGHALTITUDE, {
      ...searchForm.value,
    });
    store.setCurrentOperationType(operationTypeEnum.HIGHALTITUDE);
  ...
  const { guideVisible, guideItems, handleOperationGuide } = useOperationGuide(
    operationTypeEnum.HIGHALTITUDE
  );
</script>
