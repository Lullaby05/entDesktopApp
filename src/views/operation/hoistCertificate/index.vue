    ...
    <TableListCard>
      <template #headerInput>
        <a-form :model="tempSearchForm" layout="inline">
          ...
          <a-form-item field="operationLevel" label="吊装作业级别">
            <a-select
              v-model="tempSearchForm.operationLevel"
              allow-search
              allow-clear
              placeholder="请选择吊装作业级别"
            >
              <a-option
      ...
      <template #primaryBtn>
        <a-button
          v-permission="'HoistCertificateAdd'"
          type="primary"
          @click="handleAdd"
        ...
        >
          ...
          <template #option="{ record }">
            <div>
              ...
              >
              <a-button
                v-permission="'HoistCertificateHandling'"
                type="text"
                size="mini"
...
<script lang="ts" setup name="HoistCertificateIndex">
  ...
  getWorkUnitList(operationTypeEnum.HOIST).then((res: any) => {
  ...
  getOperationTypeEnum(operationTypeEnum.HOIST).then((res) => {
    fireStatusDict.value = changeMapToDict(res.data);
  });
  getMapDictionaryAPI('DZ_LEVEL').then((res) => {
  ...
  const getTableList = async () => {
    const { data } = await getCertificateList(operationTypeEnum.HOIST, {
      ...searchForm.value,
    });
    store.setCurrentOperationType(operationTypeEnum.HOIST);
