  ...
  <div class="container">
    <div class="header">
      <certificateHeader
        ...
        :id="(id as string)"
        print-url="HoistCertificateDetail"
      />
    </div>
    <div class="process">
...
<script lang="ts" setup>
  ...
  const initData = async () => {
    ...
    // 获取数据
    processKey.value = (await getProcessKeyEnum(operationTypeEnum.HOIST)).data;
  ...
  const handleTabList = () => {
    ...
    if (
      formProcessData.value.progress[formProcessData.value.progress.length - 1]
        .operationType === 'close'
    ) {
      // 特殊情况关闭作业
      tempTab.push({
