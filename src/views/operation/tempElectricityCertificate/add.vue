    ...
    <a-card class="content-card">
      <div class="header">
        <icon-left @click="router.back()" /><span class="title"
          >{{ type === 'edit' ? '编辑' : '新增' }}临时用电安全作业证</span
        >
      </div>
...
<script lang="ts" setup>
  ...
  onBeforeMount(async () => {
    if (type === 'edit') {
      ...
      // 重新生成
      const { data } = await getFormModel(operationTypeEnum.TEMPELECTRICITY);
      defaultData = cloneDeep(data);
      const res = await getCertificateDetail(id as string);
      const newestCode = (
        await getNewCertificateCode(operationTypeEnum.TEMPELECTRICITY)
      ).data;
      saveType = 'reApply';
      dept.value = await getUserDepts(userStore.enterPriseUserId);
