<template>
  <div class="auditContainer">
    <div class="audit-part" v-if="props.status === 'edit'">
      <a-form :model="formData" ref="auditForm">
        <a-form-item
          :label="props.text + '结果'"
          field="result"
        ...
        <!-- <a-form-item label="添加图片" field="imageFile">
          <image-upload
            class="image-upload"
        ...
        <a-form-item label="添加附件" field="uploadFile">
          <file-upload
    ...
    >
      <cardDisplay v-if="cardItems?.items.length" :card-items="cardItems" />
    </div>
    <div
      class="audit-part"
      v-if="
...
<script lang="ts" setup>
  ...
  import ImageUpload from '@/components/form-design/components/components/ImageUpload.vue';
  import FileUpload from '@/components/form-design/components/components/FileUpload.vue';
  import cardDisplay from '@/views/pitfall/snapshot/components/cardDisplay.vue';
  import { ref } from 'vue';
  import { computed } from 'vue';
  import { TableData } from '@arco-design/web-vue';
  import { getNeedSignByTaskId } from '@/api/org';
