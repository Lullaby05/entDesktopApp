      ...
      >
        <div class="audit-item-title">
          ...
          <span class="audit-item-title-text">{{
            '第' + (formProcessData.length - index) + '次审查结果'
          }}</span>
        </div>
...
<script lang="ts" setup>
  ...
  const personInfoIndex = (props.originalProgress as any[]).findLastIndex(
    (item: any) => item.props.processKey === 'review'
  );
  ...
  // 特殊情况，需要多人会签，某一个人退回了但是另一个人还未审批，导致过后查询的时候会查询到这一条未审批的数据
  const currentProcess = computed<any>(() => {
    // 只找最近的一个processKey为review的
  ...
  for (let i = 0; i < tempProgress.length; i++) {
    if (tempProgress[i].processKey === 'review' && tempProgress[i].result) {
  ...
  const status = computed<string>(() => {
    ...
    if (store.needAnalysis) {
      condition = props.certificateStatus === fireStatusEnum.书面审核中;
    } else {
      condition = props.certificateStatus === blindPlateStatusEnum.书面审核中;
    }
            ...
            return [
              {
                ...
                text: '确定',
                type: 'primary',
                permission: store.currentOperationTypeName + 'Review',
              },
            ];
          }
        }
