      ...
      >
        ...
        <div v-else class="detail-content">
          ...
          <div
            ...
            v-if="item.result && fuzzyKeySearch(item.processKey)"
          >
            <span
              class="font-weight"
              :style="{
              ...
              >{{ fuzzyKeySearch(item.processKey)
...
<script lang="ts" setup>
  ...
  progress = progress
    ...
    .filter((ele: any) => {
      if (ele.approvalMode === 'OR') {
        return ele.result || ele.lastNode;
      }
      return true;
    });
  ...
  const fuzzyKeySearch = (searchKey: string) => {
    if (!searchKey) return '';
    const [key, value] = Object.entries(processKeyMap).find(([key]) =>
