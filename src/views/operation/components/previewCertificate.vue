  ...
  <div class="preview-certificate" v-loading="loading">
    <div class="btn-group">
      <div v-if="props.status === 'preview'">
        <a-button
          v-if="pauseVisible"
          v-permission="store.currentOperationTypeName + 'Pause'"
          type="primary"
        ...
        <a-button
          v-if="closeVisible"
          v-permission="store.currentOperationTypeName + 'Close'"
          status="danger"
        ...
        <a-button
          v-if="startVisible"
          v-permission="store.currentOperationTypeName + 'Start'"
          type="primary"
...
<script lang="ts" setup>
  ...
  const pauseVisible =
    condition &&
    props.guardianUserId == userStore.enterPriseUserId &&
    !props.formProcessData.isTimeout; // 作业中才展示作业暂停
  const closeVisible =
    condition && props.guardianUserId == userStore.enterPriseUserId; //作业中的监护人才可以关闭作业
  const startVisible =
    props.certificateStatus === blindPlateStatusEnum.已安全交底 &&
    props.guardianUserId == userStore.enterPriseUserId &&
    !props.formProcessData.isTimeout; //已安全交底的监护人才可以开始作业(只有盲板抽堵才需要开始作业)
  ...
  const startOperation = async () => {
    const params = {
      ...
      operationBeginTime: formatDate(formData.value.StartTime),
      updateSign: true,
    };
  ...
  const formatFormData = computed(async () => {
    ...
    val.owner =
      props.formProcessData.staterUser && props.formProcessData.staterUser.name;
    val.startTime =
