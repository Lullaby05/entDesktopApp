  ...
  <div class="container">
    <div class="header">
      <certificateHeader
        ...
        :id="(id as string)"
        print-url="ConfinedSpaceCertificateDetail"
      />
    </div>
    <div class="process">
...
<script lang="ts" setup>
  ...
  const statusToKey: any = {
    [fireStatusEnum.待分析]: 'analyse',
    [fireStatusEnum.分析不合格]: 'analyse',
    [fireStatusEnum.已撤回]: 'apply',
    [fireStatusEnum.书面审核中]: 'review',
    [fireStatusEnum.书面审核不通过]: 'apply',
    [fireStatusEnum.现场核查中]: 'siteCheck',
    [fireStatusEnum.现场核查不通过]: 'apply',
    [fireStatusEnum.安全交底中]: 'safeDisclosure',
    [fireStatusEnum.已安全交底]: 'safeDisclosure',
    [fireStatusEnum.作业中]: 'operationPreview',
    [fireStatusEnum.作业暂停]: 'operationPause',
    [fireStatusEnum.已关闭]: 'acceptance',
  };
  ...
  const initData = async () => {
    ...
    // 获取数据
    processKey.value = (
      await getProcessKeyEnum(operationTypeEnum.CONFINEDSPACE)
    ).data;
  ...
  const handleTabList = () => {
    ...
    const tempTab: any[] = [
      {
        ...
        name: '作业预览信息',
        visible: (status.value as string) > fireStatusEnum.已安全交底,
      }, // 预览信息要在作业中才会有
      {
        code: 'operationPause',
        name: '作业暂停信息',
        visible:
          (status.value as string) > fireStatusEnum.已安全交底 && pauseVisible,
      }, // 暂停信息在作业暂停或作业中都会有
      {
        code: 'operationClose',
        name: '作业关闭信息',
    ...
    const tempTabList: any[] = processList.value.slice(0, currentStep.value);
    if (status.value === fireStatusEnum.作业暂停) {
