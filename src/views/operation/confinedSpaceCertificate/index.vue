...
<template>
  <div class="container">
    <TableListCard>
      <template #headerInput>
        <a-form :model="tempSearchForm" layout="inline">
          ...
          <a-form-item field="status" label="作业证状态">
            ...
            >
              <a-option
                v-for="item in confinedSpaceStatusDict"
                :key="item.dictValue"
      ...
      <template #primaryBtn>
        <a-button
          v-permission="'ConfinedSpaceCertificateAdd'"
          type="primary"
          @click="handleAdd"
        ...
        >
          ...
          <template #option="{ record }">
            <div>
              ...
              >
              <a-button
                v-permission="'ConfinedSpaceCertificateHandling'"
                type="text"
                size="mini"
...
<script lang="ts" setup name="ConfinedSpaceCertificateIndex">
  ...
  getWorkUnitList(operationTypeEnum.CONFINEDSPACE).then((res: any) => {
  ...
  const confinedSpaceStatusDict = ref<any>([]);
  getOperationTypeEnum(operationTypeEnum.CONFINEDSPACE).then((res) => {
    confinedSpaceStatusDict.value = changeMapToDict(res.data);
  });
  ...
  const getTableList = async () => {
    const { data } = await getCertificateList(operationTypeEnum.CONFINEDSPACE, {
      ...searchForm.value,
    });
    store.setCurrentOperationType(operationTypeEnum.CONFINEDSPACE);
