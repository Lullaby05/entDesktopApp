    ...
    <TableListCard>
      <template #headerInput>
        <a-form :model="tempSearchForm" layout="inline">
          ...
          <a-form-item field="operationType" label="作业类别">
            <a-select
      ...
      <template #primaryBtn>
        <a-button
          v-permission="'BlindPlateCertificateAdd'"
          type="primary"
          @click="handleAdd"
        ...
        >
          ...
          <template #option="{ record }">
            <div>
              ...
              >
              <a-button
                v-permission="'BlindPlateCertificateHandling'"
                type="text"
                size="mini"
...
<script lang="ts" setup name="BlindPlateCertificateIndex">
  ...
  getWorkUnitList(operationTypeEnum.BLINDPLATE).then((res: any) => {
  ...
  getOperationTypeEnum(operationTypeEnum.BLINDPLATE).then((res) => {
    fireStatusDict.value = changeMapToDict(res.data);
  });
  getMapDictionaryAPI('MB_WORK_TYPE').then((res) => {
  ...
  const getTableList = async () => {
    const { data } = await getCertificateList(operationTypeEnum.BLINDPLATE, {
      ...searchForm.value,
    });
