      ...
      <template #headerInput>
        <a-form ref="selectFormDom" :model="tempSearchForm" layout="inline">
          <a-form-item field="hazardDesc" label="隐患描述">
            <a-input
              v-model.trim="tempSearchForm.hazardDesc"
              placeholder="请输入隐患描述"
              allow-clear
            ></a-input>
          </a-form-item>
          <a-form-item field="hazardAddr" label="隐患位置">
            <a-input
              v-model.trim="tempSearchForm.hazardAddr"
              placeholder="请输入隐患位置"
              allow-clear
            ></a-input>
          </a-form-item>
          ...
          <a-form-item field="rectifyCount" label="整改次数">
            <a-input
              v-model.trim="tempSearchForm.rectifyCount"
              placeholder="请输入整改次数"
              allow-clear
            ></a-input>
          </a-form-item>
...
<script lang="ts">
  ...
  import { getOrganizationTreeAPI } from '@/api/enterpriseManage';
  ...
  import { getPitfallList } from '@/api/pitfall';
...
<script lang="ts" setup>
  ...
  const toEditPage = (type: 'toDetail', row?: any) => {
    ...
    router.push({
      name: 'PitfallDetail',
      query: {
        type: row.rectifyFlag ? 'immediate' : type,
        title: obj[type],
