<!-- 隐患趋势图表 -->
<template>
  <div class="event-num-tendency-container">
    <div class="event-num-tendency-echarts" ref="echartsRef"></div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const currentData = computed<any>(() => {
    const breakdown: any = [];
    const emergency: any = [];
    const hiddenDanger: any = [];
    data.value.BreakdownEvent.forEach((item: any) => {
      breakdown.push([item.abscissa, item.ordinate]);
    });
    data.value.emergencyEvent.forEach((item: any) => {
      emergency.push([item.abscissa, item.ordinate]);
    });
    data.value.hiddenDangerEvent.forEach((item: any) => {
      hiddenDanger.push([item.abscissa, item.ordinate]);
    });
    return {
      breakdown,
      emergency,
...
<style lang="less" scoped>
  .event-num-tendency-container {
    width: 100%;
    height: 100%;
  }
  .event-num-tendency-echarts {
    width: 100%;
    height: 100%;
  }
</style>
