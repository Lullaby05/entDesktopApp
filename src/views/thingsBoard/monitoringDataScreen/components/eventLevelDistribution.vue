...
<script lang="ts" setup>
          ...
          ...[
            {
              ...
              yAxisIndex: index,
              barWidth:
                props.type === 'event' ? findFontSize(15) : findFontSize(8),
              stack: name[index] + i,
              color: {
                type: 'linear',
                x: 0,
            ...
            valueList.forEach((item, index) => {
              dataStr += `<div>
                  <div style="display: flex;
                              ...
                              font-weight: 400;
                              width: 6.77vw;">
                    <div style="width: 0.71rem;height: 0.71rem;margin-right: 0.57rem;background:linear-gradient(270deg, ${
                      item.color.colorStops[1].color
                    } 0%, ${item.color.colorStops[0].color} 100%);"></div>
  ...
  const initChart = () => {
    ...
    myChart.value.on('click', (params: any) => {
      const componentIndexObj = {
        1: [2, 4, 6, 8], // 应急
        2: [1, 7], // 隐患
        3: [0, 3, 5], // 设备故障
      };
      let eventType = 0;
      Object.entries(componentIndexObj).forEach(([key, value]) => {
        if (value.includes(params.dataIndex)) {
          eventType = Number(key);
        }
      });
      let ids: string[] = [];
      data.value.eventLevelDistribution.forEach((item: any) => {
        if (item.key.split('-')[0] == eventType) {
          ids.push(...item.ids);
        }
      });
      emits('handleClickEchart', {
        title: params.name + '明细',
