<!-- 在线设备 -->
<template>
  <div class="equipment-warning-rate-container">
    <div class="equipment-warning-chart" ref="echartsRef"> </div>
    <div class="description-text">
      <span class="rate-text">告警设备</span>
    </div>
  </div>
</template>
<script lang="ts" setup>
  ...
  const currentData = computed<any>(() => {
    ...
    const result = new Decimal(data.value.alarms)
      .div(total)
      .mul(100)
      .toNumber();
    return result;
...
<style lang="less" scoped>
  .equipment-warning-rate-container {
    width: 11rem;
    height: 100%;
    display: flex;
    align-items: center;
