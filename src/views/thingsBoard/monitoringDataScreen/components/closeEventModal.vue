    ...
    <div class="close-modal-container">
      ...
      <div class="content">
        <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
          <a-form-item field="summary" label="事件总结">
            <a-textarea
              :max-length="500"
              :auto-size="{
                minRows: 6,
          ...
          <a-form-item field="summaryAgreement" label="附件">
            <a-space>
              <a-upload
                v-model:file-list="form.summaryAgreement"
                action=""
...
<script lang="ts" setup>
  ...
  import { addEventRecord, closeEvent } from '@/api/thingsBoard';
  ...
  const rules = {
    summary: [{ required: true, message: '请输入事件总结', trigger: 'blur' }],
  };
  const id = ref<string>('');
  ...
  const handleBeforeOk = async () => {
    ...
    const params = {
      ...form.value,
      summaryAgreement: form.value.summaryAgreement.map((item: FileItem) => {
        return {
          filePath: item.url,
          fileName: item.name,
    ...
    await closeEvent(id.value, params);
  ...
  const { headers, action, customRequest, onSuccess, beforeRemove } = useUpload(
    form,
    'summaryAgreement'
  );
