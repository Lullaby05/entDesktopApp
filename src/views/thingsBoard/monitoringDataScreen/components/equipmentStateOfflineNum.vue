<!-- 在线设备 -->
<template>
  <div class="equipment-offline-rate-container">
    <div class="equipment-offline-chart" ref="echartsRef"> </div>
    <div class="description-text">
      <span class="rate-text">离线设备</span>
    </div>
  </div>
</template>
<script lang="ts" setup>
  ...
  const currentData = computed<any>(() => {
    ...
    const result = new Decimal(data.value.offline)
      .div(total)
      .mul(100)
      .toNumber();
    return result;
...
<style lang="less" scoped>
  .equipment-offline-rate-container {
    width: 11rem;
    height: 100%;
    display: flex;
    align-items: center;
