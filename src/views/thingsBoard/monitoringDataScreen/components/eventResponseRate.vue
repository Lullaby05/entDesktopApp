<!-- 隐患趋势图表 -->
<template>
  <div class="event-response-rate-container">
    <div class="event-response-rate-echarts" ref="echartsRef"></div>
    <div class="event-response-rate-content">
      <div class="event-response-rate-description">事件响应率</div>
      <div class="event-response-rate-num">{{ Math.round(currentData) }}%</div>
    </div>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  const currentData = computed<any>(() => {
    const { all = 0, wait = 0 } = data.value;
    if (all === 0) {
      return 0;
    }
    return new Decimal(1)
      .sub(new Decimal(wait).div(new Decimal(all)))
      .mul(100)
      .toNumber();
  });
  const startColor = ref('#3694ff');
  const endColor = ref('#3694ff');
...
<style lang="less" scoped>
  .event-response-rate-container {
    ...
    .event-response-rate-content {
      display: flex;
      flex-direction: column;
      align-items: center;
      gap: 0.7rem;
    }
    .event-response-rate-num {
      font-family: 'Roboto';
      color: #00ddff;
      font-size: 2.14rem;
      font-weight: 700;
