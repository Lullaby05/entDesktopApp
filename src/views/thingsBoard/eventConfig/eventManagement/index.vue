...
<template>
  <div class="container">
    <a-tabs
      v-model:active-key="selectForm.eventType"
      type="card-gutter"
      :hide-content="true"
      @change="
        () => {
          thingsBoardStore.$patch({ emergencyListTab: selectForm.eventType });
          selectForm.pageNo = 1;
          getTableList();
        }
      "
  ...
  const getTableList = async () => {
    const params = {
      ...
      eventLevel: selectForm.value.typeNLevel?.split('-')[1]
        ...
        : undefined,
      eventStatusList:
        selectForm.value.eventStatus !== undefined &&
        selectForm.value.eventStatus !== '' &&
        selectForm.value.eventStatus !== null
          ? [selectForm.value.eventStatus]
          : [0, 1],
    };
