...
globalProps.$err = (err, def) => {
  try {
    globalProps.$message.error(err.response.data.message || err.response.data);
  } catch (e) {
...
globalProps.$ok = (rsp, def) => {
  return globalProps.$message.success(rsp.data ? rsp.data : def);
};
...
globalProps.$timeCoverStr = function (s, e) {
  ...
  if (days > 31 && end.month() - start.month() >= 2) {
    //将日期推至上月求差
    days = end.diff(
      start.add(end.month() - start.month() - 1, 'month'),
      'days'
    );
  }
