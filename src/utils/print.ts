...
export function bindVar(
  ...
  printDom.innerHTML = printTemplate.replace(varExp, (mc) => {
    if (isNode(mc)) {
      const result = progressData.filter((item: any) => {
        if (
          item.nodeId === mc.substring(2, mc.length - 1).trim() &&
          item.result
        ) {
          return true;
        }
      ...
      returnVal += `<div style="display: block;text-align: center; font-size: 20px; font-weight: bold; height: 30px;">${
        result[0] && result[0].result
          ? result[0].result === 'agree'
            ? '通过'
            : '不通过'
          : ''
      }</div>`;
      let signText = '';
      // 可能会有多人会签的情况，也可能会有多人或签的情况
      result.forEach((item: any) => {
        signText += `<div style="display: flex; align-items: center; justify-content: flex-end;" ><span>签名：</span>${
          item && item.signature
            ? `<img style="width: 100px; height: 50px; display: inline-block;padding: 2px;object-fit: contain;" src="${item.signature}"/>`
            : '<span style="margin-left: 40px;"></span>'
        }`;
        signText += `<span>${
          item && item.finishTime
            ? formatDate(item.finishTime, 'yyyy年MM月DD日HH时mm分')
            : '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;年&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;月&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;日&nbsp;&nbsp;&nbsp;&nbsp;时&nbsp;&nbsp;&nbsp;&nbsp;分'
        }</span></div>`;
      });
      returnVal += `<div style="display: flex;flex-direction: column;">${signText}</div>`;
