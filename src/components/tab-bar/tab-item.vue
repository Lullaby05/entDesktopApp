<template>
  ...
  >
    <span
      ...
      :class="{
        'link-activated':
          itemData.path === $route.path || itemData.path === activeLink,
      }"
      @click="goto(itemData)"
    >
      <span class="tag-link">
...
<script lang="ts" setup>
  ...
  router.afterEach((to, from) => {
    ...
    if (
      ...
      if (from.meta.hideInMenu) {
        return;
      }
      // 这个时候tag的link-activated不需要改变
      activeLink.value = from.path;
    } else {
      activeLink.value = '';
    }
  });
  ...
  const tagClose = (tag: TagProps, idx: number) => {
    ...
    if (
      props.itemData.path === route.path ||
      activeLink.value === props.itemData.path
    ) {
