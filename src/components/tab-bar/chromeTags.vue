<template>
  <div class="tabs">
    <ul class="tabrow" style="-webkit-app-region: no-drag">
      <li
        v-for="(tag, index) in modelList"
        :key="tag.key"
        :class="{
          'selected': nowKey === tag.name + tag.key,
          'only-tag': modelList.length === 1,
        }"
        @click="click(tag.name + tag.key)"
      >
        <tabItem
          :key="tag.key"
          :index="index"
          :item-data="tag"
          @tag-click="(key:string)=>click(key)"
        />
      </li>
    </ul>
  </div>
...
<script lang="ts" setup>
  ...
  listenerRouteChange((route: RouteLocationNormalized) => {
    ...
    if (
      !route.meta.noAffix &&
      !tagList.value.some(
        (tag) => tag.fullPath === route.fullPath && tag.modelName === key
      )
    ) {
