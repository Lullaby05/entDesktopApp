        ...
        <div class="org-items" :style="type === 'role' ? 'height: 350px' : ''">
          ...
          >
            ...
            <div v-if="org.type === 'dept'">
              ...
              >
                <icon name="iconfont2 icon-map-site"></icon>下级
              </span>
            </div>
            ...
            <div style="display: inline-block" v-else>
              <icon name="iconfont2 icon-bumen"></icon>
              <span class="name">{{ org.name }}</span>
            </div>
          </div>
        </div>
      ...
      <list v-model="loading" finished error-text="请求失败，点击重新加载">
        ...
        >
          ...
          <template #right-icon v-if="org.type === 'dept'">
            ...
            >
              <icon name="iconfont2 icon-map-site"></icon>
              <span> 下级</span>
            </div>
          </template>
        </cell>
  ...
  export default {
    ...
    methods: {
      ...
      async selectOk() {
        ...
        if (
          this.type === 'user' &&
          (this.config.title.includes('作业负责') ||
            this.config.title.includes('监护') ||
            this.config.title.includes('完工验收') ||
            this.config.title.includes('安全交底'))
        ) {
          const { id, deptId, deptName, name } = this.select[0];
          const result = await this.judgeBinding({
            record: { id, userName: name, deptId, deptName },
          });
          ...
          if (result.hasBindingUser) {
            if (
              ...
              const keyObjPC = {
                operationMasterId: `${this.currentOperationTypeNameSimple}Index,${this.currentOperationTypeName}`,
                guardianUserId: `${this.currentOperationTypeName}Close,${
                  this.currentOperationTypeName
                }Pause${
                  this.currentOperationType === operationTypeEnum.BLINDPLATE
                    ? `,${this.currentOperationTypeName}Start`
                    : ''
                }`,
              };
              const titleObjPC = {
                安全交底: `${this.currentOperationTypeName}SafeDisclosure`,
                完工验收: `${this.currentOperationTypeName}Acceptance`,
              };
              const keyObjWX = {
                operationMasterId: `${this.currentOperationTypeNameSimple}`,
                guardianUserId: `${this.currentOperationTypeNameSimple}Close,${
                  this.currentOperationTypeNameSimple
                }Pause${
                  this.currentOperationType === operationTypeEnum.BLINDPLATE
                    ? `,${this.currentOperationTypeNameSimple}Start`
                    : ''
                }`,
              };
              const titleObjWX = {
                安全交底: `${this.currentOperationTypeNameSimple}SafeDisclosure`,
                完工验收: `${this.currentOperationTypeNameSimple}Acceptance`,
              };
              const moduleNamesPC =
              ...
              const data = await this.judgeHasPermission({
                id,
                moduleNamesPC: moduleNamesPC || '',
                moduleNamesWX: moduleNamesWX || '',
              });
              if (!data) return;
            }
          } else if (!result) {
  ...
  .picker {
    ...
    .org-items {
      ...
      :deep(.org-item) {
        ...
        &:hover {
          background: var(--color-bg-active1);
        }
      }
    }
  }
