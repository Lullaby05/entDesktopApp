<template>
  <el-container style="height: calc(100vh - 206px)">
    <el-aside>
        ...
        <div>
          ...
          >
            ...
            >
              ...
              <span style="font-size: 13px; color: var(--color-text-grey)"
                >预览表单</span
              >
            </div>
          </el-tooltip>
          ...
          <div :class="{ bd: showMobile }">
            <div :class="{ 'form-content': showMobile }">
              <div class="form">
                <div
                  class="form-item"
                  v-if="defaultItems.length"
                  v-for="(item, index) in defaultItems"
                  :key="index"
                >
                  <div class="form-header">
                    <p
                      ><span v-if="item.props.required">*</span
                      >{{ item.title }}</p
                    >
                    <form-design-render :config="item" />
                  </div>
                </div>
                <div class="tip" v-show="otherItems.length === 0 && !isStart">
                  👈 请在左侧选择控件并拖至此处
                </div>
                <draggable
                  class="drag-from"
...
<script>
  ...
  import FormComponentConfig from '@/components/form-design/components/FormComponentConfig.vue';
  import { baseComponents } from '@/components/form-design/components/ComponentsConfigExport';
  import FormConfig from '@/components/form-design/components/config/FormConfig.vue';
  ...
  export default {
    ...
    computed: {
      ...
      otherItems: {
        get() {
          return this.forms.filter((ele) => !ele.props.default);
        },
        set(val) {
          this.forms = [...this.defaultItems, ...val];
        },
      },
      defaultItems() {
        return this.forms.filter((ele) => ele.props.default);
    ...
    methods: {
      ...
      showPrint() {
        ...
        this.printContent = this.printTemplate.replace(varExp, (mc) => {
          return (
            this.formatFormData[mc.substring(2, mc.length - 1).trim()] || mc
          );
        });
        this.$nextTick(() => {
      ...
      del(index) {
        this.$confirm(
          ...
          await this.forms.splice(index + this.defaultItems.length, 1);
        });
      },
    ...
    mounted() {
      console.log('this.from:', this.$store.state);
    },
  };
</script>
