  ...
  <div>
    ...
    <div v-else-if="mode === 'MOBILE' && !readonly">
      ...
      >
        <div style="width: 100px; display: flex; align-items: center">
          ...
          <avatar
            :size="30"
            :name="row.startUser.name"
            :src="row.startUser.avatar"
          ></avatar>
        </div>
        <ellipsis
          style="font-size: 0.9rem"
          :content="`${row.createTime.substring(5, 16)} 提交的 - ${row.name}`"
        />
    ...
    <div v-else-if="mode === 'PC' && !readonly">
        ...
        <div v-loading="loading">
          ...
          >
            ...
            <el-table-column
              show-overflow-tooltip
              :prop="
                type === 'SUBMIT' || type === 'CC' ? 'startTime' : 'createTime'
              "
              label="提交时间"
              min-width="120px"
            ></el-table-column>
            ...
            <template v-else-if="type === 'DONE'">
              ...
              <el-table-column prop="status" label="处理结果">
                <template v-slot="scope">
                  <el-tag
                    v-if="scope.row.taskResult"
                    :type="taskStatus[scope.row.taskResult].type"
                  >
            ...
            <el-table-column
              show-overflow-tooltip
              prop="taskName"
              :label="type === 'DONE' ? '目标节点' : '当前节点'"
            ></el-table-column>
            <el-table-column
              prop="status"
              v-if="type !== 'DONE' && type !== 'UNDO'"
              label="审批状态"
            >
              <template v-slot="scope">
            ...
            <el-table-column prop="status" label="操作">
              <template v-slot="scope">
                <el-button
                  type="primary"
                  link
                  @click.stop="selectProcess(scope.row)"
                  >选择</el-button
                >
              </template>
            </el-table-column>
      ...
      >
        ...
        <ellipsis
          style="font-size: 0.9rem"
          :content="`${row.createTime} 提交的 - ${row.name}`"
        />
        <icon name="el-icon-close" @click.stop="_value.splice(i, 1)"></icon>
      </div>
    </div>
