...
<script>
  ...
  import { getSafetyMeasureSmiple } from '@/api/operation/safetyMeasure';
  ...
  export default {
    ...
    created() {
      ...
      // 获取所有人员列表
      getStaffDataAllAPI({ enterpriseId: this.userInfo.enterpriseId }).then(
        (res) => {
    ...
    methods: {
      async loadSecurityMeasure() {
        ...
        const { data } = await getSafetyMeasureSmiple(
          store.currentOperationType
        );
      ...
      validate(call) {
        if (this.mode === 'PC') {
          if (this.rowLayout) {
            ...
            setTimeout(
              () => call(success === this._value.tableData.length),
              500
            );
          }
        } else {
...
<style lang="less" scoped>
  ...
  .el-table {
    --el-table-header-bg-color: var(--bg1);
    --el-table-tr-bg-color: var(--color-bg-2);
  }
</style>
