  ...
  <div>
    ...
    <template v-if="formConfig.ruleType === 'SIMPLE'">
      ...
      <div>
        <div
          class="w-form-rule"
          v-for="(rule, i) in formConfig.rules"
          :key="'rule_' + i"
          @click="editRule(i, rule)"
        >
          <div class="w-form-rule-content"></div>
          <icon
            name="el-icon-close icon"
            v-if="!rule.default"
    ...
    <w-dialog
      v-model="ruleDialog"
      :title="`${isAddRule ? '新增' : '编辑'}表单联动规则`"
      @ok="confirmRule"
    >
      <template v-if="formConfig.ruleType === 'SIMPLE'">
        <el-alert
          type="success"
          description="当满足如下条件关系时，执行下方一系列联动动作"
          close-text="知道了"
          style="margin-bottom: 10px"
        />
        <condition-config
          class="w-rule-conditions"
        ...
        <action-config
          :options="formItems"
          v-model="selectedRule.action.other"
