<template>
  <div class="w-rule-actions">
    ...
    >
    <div
      v-for="(action, i) in _value"
      :key="'action' + i"
      class="w-rule-action"
    >
      <icon name="el-icon-delete icon" @click="_value.splice(i, 1)"></icon>
      <el-select
        style="width: 150px; margin-right: 10px"
        v-model="action.type"
        placeholder="选择动作类型"
        @change="action.targets.length = 0"
      >
      ...
      <template v-else>
        ...
        <form-design-render
          v-if="action.targets.length > 0"
          mode="PC"
          v-model="action.value"
          :config="formItemMap.get(action.targets[0])"
        />
      </template>
    </div>
  </div>
