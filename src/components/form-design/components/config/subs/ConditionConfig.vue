          ...
          >
            ...
            <div class="w-condition-box-compare">
              {group.children.length > 1
                ? [
                    ...
                    >
                      <icon
                        onClick={() => this.addBrother(group)}
                        name="icon el-icon-plus"
                      ></icon>
                    </el-tooltip>,
                  ]
                : ''}
            ...
            {isRoot ? (
              <el-tooltip effect="dark" content="添加并列条件" placement="top">
                <icon
                  onClick={() => this.addBrother(group)}
                  name="el-icon-plus icon root-add"
                ></icon>
              </el-tooltip>
            ) : (
              ''
            ...
            <div
              class={[
                ...
                isRoot ? 'w-condition-box-line-none' : '',
              ]}
            ></div>
            <div
              class={[
                'w-condition-box-content',
                group.children.length > 1 ? 'w-condition-box-content-none' : '',
              ]}
            >
              {group.children.length > 1
                ? group.children.map((cd, i) => {
                    //子条件项
                    return this.getGroupDom(
                      cd,
                      false,
                      this.getCoverPos(i, group.children.length),
                      group,
                      i
                    );
                  })
                : [
                    ...
                    <div class="w-condition-options">
                      ...
                      >
                        <icon
                          onClick={() => this.delChild(parent, i)}
                          name="el-icon-delete icon"
                        ></icon>
                      </el-tooltip>
                    </div>,
                  ]}
