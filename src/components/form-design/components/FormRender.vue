<template>
  ...
  <el-form
    ...
    :model="_value"
    :disabled="disabled"
    :label-width="config.labelWidth || 225"
  >
    <div
      ...
      loadFormConfig(forms, rules) {
        ...
        forms.forEach((item) => {
          ...
          if (item.name === 'SpanLayout' || item.name === 'ModuleBlock') {
                ...
                this.rules[item.id] = [
                  {
                    ...
                    validator: (rule, value, callback) => {
                      ...
                      if (
                        ...
                        this._value[this.operationLevelItemId] === '3'
                      ) {
                        if (
                          +new Date(endTime) - +new Date(startTime) >
                          72 * 60 * 60 * 1000
            ...
            } else if (item.name === 'MultilevelLink' && this.showItem(item)) {
              this.rules[item.id] = [
                {
                  type: ValueType.getValidType(item.valueType),
                  ...
                  validator: (rule, value, callback) => {
                    ...
                    this._value[item.id] = value.filter((obj) => {
                      // 检查对象的所有属性值是否都不为空
        ...
        (actions || []).forEach((action) => {
          ...
          switch (action.type) {
            ...
            case 'DISABLE':
              ...
              break;
            case 'UPDATE':
              action.targets.forEach((tg) =>
                this.updateField(tg, action.value)
              );
              break;
            case 'ENABLE':
              action.targets.forEach((tg) =>
                this.enableField(tg, action.value)
              );
              break;
          }
        });
